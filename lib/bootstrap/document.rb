module Bootstrap
  ...
  module Document
    ...
    def with_time_zone(method)
      date = self.send(method)
      return unless date.present?

      # see if we can grab the time_zone from the merchant
      zone = nil
      zone ||= self.time_zone if self.respond_to?(:time_zone)
      zone ||= Settings.default_time_zone

      # switch date to the time_zone
